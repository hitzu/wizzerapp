...
const createMessage = async (req: Request, res: Response) => {
  try {
    ...
    const userInConversation = await conversationUserRepository.findOne({
    ...
    if (!userInConversation) {
      throw new GeneralError(
        new Error(),
        'The user is unable to send messages in this conversation',
        403
      );
    }
...
const getMessageById = async (req: Request, res: Response) => {
  try {
    ...
    console.log(`desde el error; ${JSON.stringify(error)}`, error.code);
    res.status(error?.code ?? 500).send(error);
  }
};
